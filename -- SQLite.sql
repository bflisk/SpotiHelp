...
CREATE TABLE playlist_options (user_id INTEGER NOT NULL, playlist_id TEXT NOT NULL, auto_add TEXT NOT NULL, auto_delete TEXT NOT NULL, allow_explicit TEXT NOT NULL, row_id INTEGER NOT NULL, PRIMARY KEY(row_id), FOREIGN KEY(playlist_id) REFERENCES playlists(playlist_id));
CREATE TABLE playlist_tracks (user_id INTEGER NOT NULL, playlist_id TEXT NOT NULL, track_id TEXT NOT NULL, is_favorite TEXT NOT NULL DEFAULT "False", num_skips INTEGER NOT NULL DEFAULT 0, row_id INTEGER NOT NULL, PRIMARY KEY(row_id), FOREIGN KEY(playlist_id) REFERENCES playlists(playlist_id));
